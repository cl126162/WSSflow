...
class CorrBlock:
    ...
    def corr(fmap1, fmap2):
        ...
        corr = torch.matmul(fmap1.transpose(1,2), fmap2).float()
        # corr = torch.matmul(fmap1.transpose(1, 2), fmap2).type(torch.float16)
...
class RAFT(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.fnet = BasicEncoder(output_dim=256, norm_fn='instance', dropout=0., downsample=self.downsample)
        self.cnet = BasicEncoder(output_dim=self.hidden_dim+self.context_dim, norm_fn='instance', dropout=0., downsample=self.downsample)
        self.update_block = BasicUpdateBlock(hidden_dim=self.hidden_dim, corr_levels=self.corr_levels, corr_radius=self.corr_radius, downsample=self.downsample)
